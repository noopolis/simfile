...
export interface SpawnfileLocalEndpointProof {
  architecture: "amd64" | "arm64";
  context: string;
  endpoint_class: "local";
  transport: "fd" | "npipe" | "unix";
  version: "simfile.spawnfile-local-endpoint-proof.v1";
}

export interface ProveSpawnfileLocalEndpointInput {
  base_image?: string;
  context: string;
  cwd: string;
  docker_command?: string;
  env: NodeJS.ProcessEnv;
  spawnfile_bin: string;
  state_root: string;
}
...
export const parseSpawnfileLocalEndpointProof = (
  raw: unknown,
  expectedContext: string
): Readonly<SpawnfileLocalEndpointProof> => {
  const value = raw;
  if (!isObject(value)
    ...
    || !["fd", "npipe", "unix"].includes(String(value.endpoint.transport))
    || !isObject(value.platform)
    || value.platform.os !== "linux"
    || !["amd64", "arm64"].includes(String(value.platform.architecture))
    || !isObject(value.target_config)
    || value.target_config.context !== expectedContext
    || value.target_config.version !== "spawnfile.target-default-config.v1"
    || typeof value.target_config_digest !== "string"
    || !digest.test(value.target_config_digest)
    || !isObject(value.base_image)
    || typeof value.base_image.config_digest !== "string"
    || !digest.test(value.base_image.config_digest)) {
    return fail("Spawnfile did not prove the exact context is a local endpoint");
  }
  return Object.freeze({
    architecture: value.platform.architecture as "amd64" | "arm64",
    context: expectedContext,
    endpoint_class: "local",
    transport: value.endpoint.transport as "fd" | "npipe" | "unix",
    version: "simfile.spawnfile-local-endpoint-proof.v1",
  });
};

export const proveSpawnfileLocalEndpoint = async (
  input: ProveSpawnfileLocalEndpointInput
): Promise<Readonly<SpawnfileLocalEndpointProof>> => {
