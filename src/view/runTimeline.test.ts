...
const writeWorldRuleRun = async (): Promise<string> => {
  ...
  const worldEvents = [
    {
      event_id: "simfile:run-world-rule:1",
      seq: 1,
      type: "rule.fired",
      ...
      payload: {
        ...
        target: "agent:alice", scope: "agent:alice", tick: 5, phase: "workday",
      },
    },
  ].map(({ seq, ...event }) => ({
    version: "noopolis.causal-event.v1",
...
describe("buildRunTimeline — ordinary world rule stream", () => {
  it("keeps rule records non-waking while attributing their declared target", async () => {
    const runDir = await writeWorldRuleRun();
    try {
      const timeline = await buildRunTimeline(runDir);
      const rule = eventById(timeline, "simfile:run-world-rule:1");
      assert.equal(rule.viewClass, "other");
      assert.deepEqual(rule.subjects, ["agent:alice"]);
    } finally {
      await rm(runDir, { recursive: true, force: true });
    }
  });
