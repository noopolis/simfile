...
- `matrix.test.ts` enforces the manifest's grammar and its parity with `SYSTEMS_VIEW.md` and the generated document.

## Rules

- `matrix.ts` is authoritative. `COVERAGE.md` at the repository root is generated from it via `npm run coverage:render` (see `scripts/render-coverage.ts`); never hand-edit `COVERAGE.md`.
